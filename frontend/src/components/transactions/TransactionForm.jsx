...
export default function TransactionForm({ initial = null, categories = [], onSubmit, onClose }) {
  ...
  const submit = async (event) => {
    ...
    if (type === 'Expense') {
      if (!categoryId) nextErrors.category = 'Select a category for this expense'
    }
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="tx-amount" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">
            Amount
          </label>
          <input
            id="tx-amount"
        ...
        <div>
          <label htmlFor="tx-date" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">
            Date
          </label>
          <input
            id="tx-date"
      ...
      {type === 'Expense' ? (
        <div>
          <label htmlFor="tx-category" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">
            Category
          </label>
          <select
            id="tx-category"
        ...
        <p className="rounded-lg bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-400">
          Income transactions do not have a category.
        </p>
      )}

      <div>
        <label htmlFor="tx-description" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200">
