...
def test_create_and_list_transactions(client):
    response = client.post("/api/transactions", json={"type": "Expense", "amount": "500.00", "date": "2026-09-12", "description": "Lunch", "category_id": 1})
...
def test_monthly_income_endpoint_locks_income(client):
    response = client.post("/api/income/2026/9", json={"amount": "100000.00", "description": "Salary"})
    assert response.status_code == 201
    assert response.json()["type"] == "Income"

    response = client.get("/api/income/2026/9")
    assert response.status_code == 200
    assert response.json()["amount"] == "100000.00"

    response = client.post("/api/income/2026/9", json={"amount": "120000.00"})
    assert response.status_code == 400
    assert "locked" in response.json()["detail"]

    response = client.put("/api/transactions/1", json={"type": "Income", "amount": "120000.00", "date": "2026-09-01"})
    assert response.status_code == 400
    assert "locked" in response.json()["detail"]

    response = client.delete("/api/income/2026/9")
...
def test_create_budget_and_allocation(client):
    response = client.post("/api/budgets", json={"year": 2026, "month": 9, "overall_amount": "100000.00"})
    assert response.status_code == 201
    budget_id = response.json()["id"]

    response = client.post(f"/api/budgets/{budget_id}/allocations", json={"category_id": 1, "amount": "40000.00"})
    assert response.status_code == 201
...
def test_dashboard_endpoint(client):
    client.post("/api/transactions", json={"type": "Income", "amount": "100000.00", "date": "2026-09-01"})
...
def test_csv_round_trip(client):
    response = client.post("/api/transactions", json={"type": "Expense", "amount": "750.00", "date": "2026-09-12", "description": "Groceries", "category_id": 1})
    assert response.status_code == 201
